...
void SolveConvectionDiffusion(const Grid &G, const double tf, double dt, const unsigned short conScheme, const unsigned short timeScheme, const double alpha)
{
	...
	for(int i = 0; i < Nx; i++)
		for(int j = 0; j < Ny; j++)
		{
			phie(i, j) = 5.0 * (1 - exp(u(i, j) * G.x(i) / alpha)) / (1 - exp(u(i, j) / alpha)) + 
			0.1 * (1 - exp(v(i, j) * G.y(j) / alpha)) / (1 - exp(v(i, j) / alpha));
    ...
    while(itime * dt <= tf)
	{
		...
		if(dphi.L2Norm() < 1e-8)
		{
			std::string str_dt = std::to_string(dt);
			std::string str_Nx = std::to_string(Nx);
			std::string phi_dt = "phi_dt=";
			std::string str_fill = "_Nx=";
			std::string vtk = ".vtk";
			std::string total = phi_dt + str_dt + str_fill + str_Nx + vtk;
			char fname[total.length() + 1];
			strcpy(fname, total.c_str());
			...
			phi_err = phi - phie;
			printf("The L2Norm of the error is %12.14e", phi_err.L2Norm());
			break;
		}
	}
	// Save VTK file with timestep in it
...
int main()
{
	...
	if (pbtype == 1)
	{
		...
		double ylim1[2] = {-1, 1};
		Nx1 = Ny1 = 201;
		int numcheck = 11; // Number of CFL numbers to run
		double tf1 = 2*PI;
		double CFL[numcheck] = {1};
	...
	else if (pbtype == 2)
	{
		...
		int numcheck = 1;
		//double CFL[numcheck] = {0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45};
		double CFL[numcheck] = {0.9};
