...
void NSDriver::Run()
{
	...
	/*========================================================================================
	 * assemble the LHS for the X momentum equation Au (already defined in the structure) 
	 *========================================================================================
 	 */ 
	assembleA('U');
	/*========================================================================================
	 * assemble the LHS for the Y momentum equation Av (already defined in the structure)
	 *========================================================================================
 	 */
	assembleA('V');
...
void NSDriver::solveXMomPredictor()
{
	...
	for(i = 0; i < G.Nx(); i++){
		...
		bu(i,j) = 0;
		j = G.Ny() - 1;
		bu(i,j) = 1; //Top plate movement
	}

	S.solve(u_, Au, bu); //need to solve for u_

	momResx = bu - Au*u_; // what does this line do then? 
...
void NSDriver::solveYMomPredictor()
{
	...
	S.solve(v_, Av, bv); //need to solve for v_..
...
void NSDriver::correctFlowVars()
{
	...
	for(size_t i = 1; i < G.Nx()-1; i++)
		for(size_t j = 1; j < G.Ny()-1; j++){
			...
			u_(i, j) = u_(i, j) + ((-pc(i + 1, j) + pc(i - 1, j)) / (2 * a * dx));
			v_(i, j) = v_(i, j) + ((-pc(i, j + 1) + pc(i, j - 1)) / (2 * a * dy));
		}
}
...
void NSDriver::assembleb0(char flowVar)
{
	switch(flowVar)
	{
		case 'U':
		{
			bu0 = Fdu - 1.5 * Fcu + 0.5 * Fcu_;
			break;
		}
		case 'V':
		{
			bv0 = Fdv - 1.5 * Fcv + 0.5 * Fcv_;
			break;
		}
	}
}
...
void NSDriver::assembleA(char flowVar)
{
	...
	switch(flowVar)
	{
		...
		for(size_t i = 1; i < G.Nx()-1; i++)
			...
			{
				//Au(i,j,0) = 0;
				Au(i, j, 0) = -1.0 / (2 * dx * dx * Params.Re);
				Au(i, j, 1) = -1.0 / (2 * dy * dy * Params.Re);
				Au(i, j, 2) = (1.0 / Params.dt) + (((1.0 / (dx * dx)) + (1.0 / (dy * dy))) / Params.Re);
				Au(i, j, 3) = -1.0 / (2 * dy * dy * Params.Re);
				Au(i, j, 4) = -1.0 / (2 * dx * dx * Params.Re);
			}
				
		for(i = 0; i < G.Nx(); i++) //BCs
			for(int t = 0; t < 5; t++){
				Au(i,0,t) = (t == 2 ? 1.0 : 0.0);
				Au(i,G.Ny()-1, t) = (t == 2 ? 1.0 : 0.0); 
			}

		for(j = 0; j < G.Ny(); j++) //BCs
			for(int t = 0; t < 5; t++){
				Au(0,j,t) = (t == 2 ? 1.0 : 0.0);
				Au(G.Nx()-1,j,t) = (t == 2 ? 1.0 : 0.0);
			}
		break;
		...
		for(size_t i = 1; i < G.Nx()-1; i++)
			...
			{
				//Av(i,j,0) = 0;
				Av(i, j, 0) = -1.0 / (2 * dx * dx * Params.Re);
				Av(i, j, 1) = -1.0 / (2 * dy * dy * Params.Re);
				Av(i, j, 2) = (1.0 / Params.dt) + (((1.0 / (dx * dx)) + (1.0 / (dy * dy))) / Params.Re);
				Av(i, j, 3) = -1.0 / (2 * dy * dy * Params.Re);
				Av(i, j, 4) = -1.0 / (2 * dx * dx * Params.Re);
			}	
		for(i = 0; i < G.Nx(); i++) //BCs
			for(int t = 0; t < 5; t++){
				Av(i,0,t) = (t == 2 ? 1.0 : 0.0);
				Av(i,G.Ny()-1, t) = (t == 2 ? 1.0 : 0.0); 
			}

		for(j = 0; j < G.Ny(); j++) //BCs
			for(int t = 0; t < 5; t++){
				Av(0,j,t) = (t == 2 ? 1.0 : 0.0);
				Av(G.Nx()-1,j,t) = (t == 2 ? 1.0 : 0.0);
			}
		break;
	}
}
...
void NSDriver::assembleAp()
{
	...
	for (size_t i = 2; i < Nx-2; i++){
		...
		/*
			...
			Ap(i,j,0) = -1/(a*dx*dx);
			Ap(i,j,1) = -1/(a*dy*dy);
			Ap(i,j,2) = 2*((1/(a*dx*dx))+(1/(a*dy*dy)));
			Ap(i,j,3) = -1/(a*dy*dy);
			Ap(i,j,4) = -1/(a*dx*dx);
		}
	}
...
void NSDriver::assemblebp()
{
	...
	for (size_t i = 1; i < Nx-1; i++){
		for (size_t j = 1; j < Ny-1; j++){
			...
			xx = ee - (4*p_(i+1,j)) + (6*p_(i,j)) - (4*p_(i-1,j)) + ww ;
			yy = nn - (4*p_(i,j+1)) + (6*p_(i,j)) - (4*p_(i,j-1)) + ss ; 
			bp(i,j) = -(((u_(i+1,j)-u_(i-1,j))/(2*dx))+((xx)/(4*a*dx*dx)))-(((v_(i,j+1)-v_(i,j-1))/(2*dy))+((yy)/(4*a*dy*dy))) ;
