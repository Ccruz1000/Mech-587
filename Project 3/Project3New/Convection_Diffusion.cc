...
void computeConvectiveXFlux(Vector &Fc, const Vector &u, const Vector &v, const Grid &G)
{
	...
	double dx, dy;
	dx = G.dx(); dy = G.dy();  // Spatial Steps

	double ue, uw, un, us;  // U Velocity at east, west, north, and south faces
	double ve, vw, vn, vs; // V velocity at east, west, north, and south faces

	

	for(size_t i = 1; i < G.Nx()-1; i++)
		for(size_t j = 1; j < G.Ny()-1; j++) // Loop through all interior points
		{
			// Calculate velocity at all faces
			// East face u(i + 1/2, j)
			ue = 0.5 * (u(i + 1, j) + u(i, j)); // u velocity
			ve = 0.5 * (v(i + 1, j) + v(i, j)); // v velocity
			// West face u(i - 1//2, j)
			uw = 0.5 * (u(i, j) + u(i - i, j)); // u velocity
			vw = 0.5 * (v(i, j) + v(i - i, j)); // v velocity
			// North face u(i, j + 1/2)
			un = 0.5 * (u(i, j + 1) + u(i, j)); // u velocity
			vn = 0.5 * (v(i, j + 1) + v(i, j)); // v velocity
			// South Face u(i, j - 1/2)
			us = 0.5 * (u(i, j) + u(i, j - 1)); // u velocity
			vs = 0.5 * (v(i, j) + v(i, j - 1)); // v velocity	
...
void computeConvectiveYFlux(Vector &Fc, const Vector &u, const Vector &v, const Grid &G)
{
	...
	for(size_t i = 1; i < G.Nx()-1; i++)
		for(size_t j = 1; j < G.Ny()-1; j++) // Loop through all interior points
		{
			...
			// Calculate convective flux
			Fc(i,j) = (((ue * ve) - (uw * vw)) / dx) + (((vn * vn) - (vs * vs)) / dy);
		}
}
...
void computeDiffusiveFlux(Vector &Fd, const Vector &phi, const Grid &G)
{
	double dx, dy;
	dx = G.dx(), dy = G.dy(); // Spatial Steps
	double phi_curr; // Place holder for current position
	double dphidxe, dphidxw, dphidyn, dphidys; // East, west, north, south faces

	for(size_t i = 1; i < G.Nx()-1; i++)
		for(size_t j = 1; j < G.Ny()-1; j++) // Loop through all interior points
		{
			// Calculate values at faces and current point
			dphidxe = phi(i + 1, j);
			dphidxw = phi(i - 1, j);
			dphidyn = phi(i, j + 1);
			dphidys = phi(i, j - 1);
			...
			Fd(i,j) = (((dphidxe - (2 * phi_curr) + dphidxw)) / (dx * dx)) + (((dphidyn - (2 * phi_curr) + dphidys)) / (dy * dy));
		}
}
...
void computeGradient(Vector &gradphix, Vector &gradphiy, const Vector &phi, const Grid &G)
{
	...
	for(size_t i = 1; i < G.Nx() -1; i++)
		for(size_t j = 1; j < G.Ny()-1; j++) // Loop through all interior points
		{
			gradphix(i,j) = (phi(i + 1, j) - phi(i - 1, j)) / (2 * dx);
			gradphiy(i,j) = (phi(i, j + 1) - phi(i, j - 1)) / (2 * dy);
		}
}
