...
NSParameters::NSParameters():	dt(Config::Instance()->dt()),
	...
	/*
	This creates the Parameters which are read from the NS.inp file.
	*/
{}
...
void NSDriver::initializeVel()
{
	...
	for(size_t i = 0; i < G.Nx(); i++){
		u(i,0) = 0;
		u(i,G.Ny()-1) = 1; // Applies horizontal velocity of 1 to top boundary initially 
		// Velocity is 0 everywhere else in both u and v
	}
}
...
void NSDriver::initializePres()
{
	for(size_t i = 0; i < G.Nx(); i++)
		for(size_t j = 0; j < G.Ny(); j++)
			p(i,j) = 0;
		// Applies initial pressure of 0 everywhere
}
...
void NSDriver::correctFlowVars()
{
	...
	for(size_t i = 1; i < G.Nx()-1; i++)
		for(size_t j = 1; j < G.Ny()-1; j++){
			...
			u_(i,j) = u_(i,j) - ((1 / (2 * a * dx)) * (p_(i + 1, j) - p_(i - 1, j)));
			//u_(i, j) = u_(i, j) + Params.alphaU * uf(u)
			v_(i,j) = v_(i,j) - ((1 / (2 * a * dy)) * (p_(i, j + 1) - p_(i, j - 1)));
		}
}
...
void NSDriver::assembleAp()
{
	...
	for (size_t i = 2; i < Nx-2; i++){
		...
		// For solving NS equation, changing the rest of the code is not recommended.
			...
			Ap(1,j,0) = - 1.0/(a*dx*dx);
			Ap(1,j,1) = - 1.0/(a*dy*dy);
			Ap(1,j,2) =  2.0 / (a * dx * dx) + 2 / (a * dy * dy);
			Ap(1,j,3) = - 1.0/(a*dx*dx);
			Ap(1,j,4) = - 1.0/(a*dy*dy);
		}
	}
	for (size_t j = 2; j < Ny-2; j++){
...
void NSDriver::assemblebp()
{
	...
	for (size_t i = 1; i < Nx-1; i++){
		for (size_t j = 1; j < Ny-1; j++){
			...
			bp(i,j) = (- 1 / (4 * a * dx * dx)) * (ee - 4 * p_(i + 1, j) + 6 * p_(i, j) - 4 * p_(i - 1, j) + ww) + 
			(- 1 / (4 * a * dy * dy)) * (nn - 4 * p_(i, j + 1) + 6 * p_(i, j) - 4 * p_(i, j - 1) + ss);
